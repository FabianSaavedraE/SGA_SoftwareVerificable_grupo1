...
def updateSectionView(course_section_id):
    ...
    teachers = getAllTeachers()
    if not course_section:
        return redirect(url_for('course_instances.showCourseInstance', course_instance_id=course_section.course_instance.id))

    if request.method == 'POST':
        data = request.form
        updateSection(course_section, data)

        return redirect(url_for('course_instances.showCourseInstanceView', course_instance_id=course_section.course_instance.id))
