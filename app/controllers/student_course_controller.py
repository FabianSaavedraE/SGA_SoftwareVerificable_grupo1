...
def update_state(student_course, final_grade):
    ...
    student_course.state = APPROVED if final_grade >= APPROVED_GRADE else FAILED
    return student_course
...
def create_student_courses_from_json(data):
    ...
    for student_course in student_courses:
        ...
        if not validate_entry_can_be_loaded(
            transform_json_entry_into_processable_student_course_format(student_course),
            "student_course",
        ):
            return None
...
def create_all_student_course_entries(student_courses):
    for entry in student_courses:
        transformed_data = transform_json_entry_into_processable_student_course_format(
            entry
        )
